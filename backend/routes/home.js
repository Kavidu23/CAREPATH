...
router.post("/appointment", authenticateUser, (req, res) => {
  ...
  if (!Did || !date || !time || !type)
    return res.status(400).json({ message: "Missing fields" });
  if (!req.session.user)
    return res.status(401).json({ message: "Unauthorized" }); // Check if user is logged in
  const Pid = req.session.user.id;

  const query =
    "INSERT INTO Appointment (Pid, Did, Date, Time, Type) VALUES (?, ?, ?, ?, ?)";
...
router.get("/dashboard-status", (req, res) => {
  const queries = {
    ...
    specializations:
      "SELECT COUNT(DISTINCT Specialization) AS totalSpecializations FROM Doctor_Specialization",
