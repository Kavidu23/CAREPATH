...
router.get("/profile", authenticateUser, (req, res) => {
  if (!req.session.user)
    return res
      .status(401)
      .json({ message: "Unauthenticated, Session not available" });
...
router.get("/invoice", authenticateUser, (req, res) => {
  ...
  const queries = `
      SELECT Invoice.Pid, IssuedDate, FinalAmount,TotalAmount,PaymentStatus
      FROM Invoice
  ...
  connection.query(queries, [Pid], (error, results) => {
    ...
    if (results.length === 0) {
      return res
        .status(404)
        .json({ message: "No invoice found for this patient." });
    }
